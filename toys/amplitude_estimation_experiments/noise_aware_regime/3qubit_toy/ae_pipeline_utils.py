...
def build_large_state_preparation(objective_ry_offset: float = CANONICAL_OBJECTIVE_RY_OFFSET) -> QuantumCircuit:
    return build_state_preparation(objective_ry_offset)


def build_large_problem(objective_ry_offset: float = CANONICAL_OBJECTIVE_RY_OFFSET) -> tuple[EstimationProblem, float]:
...
def build_noise_model(scale: float, *, profile: str = NOISE_PROFILE_BASELINE) -> NoiseModel:
        ...
        for budget in budgets:
            for alg in algorithms:
                ...
                normalized_abs_error = np.asarray(
                    [
                        float(r.get("normalized_abs_error", r.get("nrmse")))
                        for r in subset
                    ],
                    dtype=float,
                )
...
def print_budget_summary(summary_rows: list[dict[str, Any]], profile_names: Iterable[str]) -> None:
    ...
    for profile_name in profile_names:
        ...
        for budget in budgets:
            ...
            b_rows.sort(
                key=lambda x: float(
                    x.get("normalized_abs_error_median", x.get("nrmse_median"))
                )
            )
            ...
            for row in b_rows:
                ...
                print(
                    "    "
                    + f"{row['algorithm']:16s} "
                    + f"NormAbsErr_med={row.get('normalized_abs_error_median', row.get('nrmse_median')):.3e} | "
...
def print_final_summary(
    ...
    for profile_name in profile_names:
        ...
        for alg in algorithms:
            ...
            nae = np.asarray(
                [
                    float(r.get("final_normalized_abs_error", r.get("final_nrmse")))
                    for r in subset
                ],
                dtype=float,
            )
            ...
            print(
                ...
                + f"NormAbsErr_med={np.nanmedian(nae):.3e} | "
...
def plot_budget_panels(
    ...
    for j, profile in enumerate(profile_names):
        ...
        for alg in algorithms:
            ...
            normalized_abs_error = np.asarray(
                [
                    float(r.get("normalized_abs_error_median", r.get("nrmse_median")))
                    for r in subset
                ],
                dtype=float,
            )
