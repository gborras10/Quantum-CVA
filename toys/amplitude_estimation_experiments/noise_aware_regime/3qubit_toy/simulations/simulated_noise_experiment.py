...
def print_transpiled_grover_depths(
    problem: Any,
    physical_backend: Any,
    transpilation_plan: Any,
) -> None:
    pass_manager = transpilation_plan.build_pass_manager(physical_backend)
    num_qubits = max(
        problem.state_preparation.num_qubits,
        problem.grover_operator.num_qubits,
    )

    print("\n=== Transpiled Grover-depth diagnostics ===")
    print("Q^k-only excludes state preparation and measurement.")
    print("Execution circuit is A + Q^k + objective measurement, matching sampler calls.")
    print(f"{'k':>3} {'K=2k+1':>7} {'Q^k depth':>11} {'Q^k 2q':>8} {'exec depth':>11} {'exec 2q':>8}")

    for k in (1, 2, 3):
        grover_circuit = QuantumCircuit(num_qubits, name=f"grover_power_k{k}")
        grover_power = problem.grover_operator.power(k)
        if hasattr(grover_power, "decompose"):
            grover_power = grover_power.decompose(reps=10)
        grover_circuit.compose(grover_power, inplace=True)
        grover_isa = pass_manager.run(grover_circuit.decompose(reps=10))

        execution_circuit = construct_measured_circuit(problem, k).decompose(reps=10)
        execution_isa = pass_manager.run(execution_circuit)

        print(
            f"{k:>3} {2 * k + 1:>7} "
            f"{grover_isa.depth():>11} {_two_qubit_gate_count(grover_isa):>8} "
            f"{execution_isa.depth():>11} {_two_qubit_gate_count(execution_isa):>8}"
        )
    print()
...
def run_experiment() -> None:
    ...
    print_transpiled_grover_depths(
        scenarios[0]["problem"],
        physical_backend,
        transpilation_plan,
    )
                ...
                try:
                    ...
                    if alg_key == "bae":
                        ...
                        result = solver.estimate(
                            problem,
                            n_shots=NUM_SHOTS,
                            max_queries=BAE_MAX_ANALYSIS_BUDGET,
                        )
                    else:
                        result = solver.estimate(
                            problem,
                    ...
                    final_normalized_abs_error = final_abs_error / a_true
                    ...
                    final_rows.append(
                        {
                            ...
                            "final_normalized_abs_error": final_normalized_abs_error,
                            "coverage": coverage,
                    ...
                    print(
                        ...
                        f"final normalized abs. error={final_normalized_abs_error:.3e} | "
