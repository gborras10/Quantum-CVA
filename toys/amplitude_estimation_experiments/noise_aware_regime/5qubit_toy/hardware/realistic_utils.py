...
DEFAULT_AE_REFERENCE_KS: tuple[int, ...] = (0, 1, 2, 4, 8)
DEFAULT_HARDWARE_ROUTING_METHOD = "sabre"
...
def build_reference_ae_circuits(
	problem: EstimationProblem,
	...
	return [
		construct_measured_circuit(problem, k)
		for k in normalized_reference_ks
	]
...
def circuit_cache_key(circuit: QuantumCircuit) -> str:
	try:
		...
		return str((circuit.num_qubits, circuit.size(), circuit.depth(), ops))


def _quality_layout_candidates(backend: Any, logical_qubit_count: int) -> list[LayoutCandidate]:
	candidates: dict[tuple[int, ...], LayoutCandidate] = {}

	for topology in ("heavy_hex_star", "linear"):
		try:
...
def choose_ae_transpilation_plan(
	backend: Any,
	problem: EstimationProblem,
	*,
	optimization_level: int = 3,
	reference_ks: Iterable[int] = DEFAULT_AE_REFERENCE_KS,
	routing_method: str | None = DEFAULT_HARDWARE_ROUTING_METHOD,
	discovery_seeds: Sequence[int] = DEFAULT_TRANSPILER_SEEDS,
	evaluation_seeds: Sequence[int] = DEFAULT_TRANSPILER_SEEDS,
) -> FixedTranspilationPlan:
	reference_circuits = build_reference_ae_circuits(problem, reference_ks=reference_ks)
	logical_qubit_count = max(
		problem.state_preparation.num_qubits,
		problem.grover_operator.num_qubits,
	)
	candidate_layouts = _quality_layout_candidates(
		backend,
...
def build_ae_pass_manager(
	...
	*,
	...
	try:
		plan = choose_ae_transpilation_plan(
			backend,
			problem,
			optimization_level=int(optimization_level),
...
def build_noise_model(scale: float) -> NoiseModel:
    """
    Effective noise model tailored specifically for the optimal linear layout 
    found in ibm_basquecountry: [134, 135, 139, 155, 154].

    Interpretation:
      - 1q noise and Readout errors calibrated from the exact averages of the 5 chosen qubits.
      - CX is treated as an effective 2q gate whose error is deducted by composing 
        the specific CZ calibration data with additional single-qubit rotations.
    """
    noise_model = NoiseModel()

    # ------------------------------------------------------------------
    # Parámetros promediados del Layout Óptimo [134, 135, 139, 155, 154]
    # ------------------------------------------------------------------

    # Error depolarizante 1q (Promedio Pauli-X / Sx de los 5 qubits elegidos)
    p1 = min(1.88e-4 * scale, 5e-3)

    # Error efectivo CX:
    # Calculado a partir del error CZ promedio en este layout (~1.55e-3) 
    # más 2 compuertas 1q necesarias para transformar CZ en CX.
    ...
    # ------------------------------------------------------------------
    # 2-qubit CX error (Construido como proxy del CZ del hardware)
    # ------------------------------------------------------------------
