...
def make_engine(n_bits: int | list[int]) -> DiscreteUnderlyingCvaEngine:
    if isinstance(n_bits, int):
        n_bits_list = [int(n_bits), int(n_bits), int(n_bits)]
    else:
...
# Our instance: 12 qubits total = 4 time qubits + (3, 2, 3) asset qubits
engine_small = make_engine([2, 2, 2])  # bins per asset = (8, 4, 8)
...
path = repo_root / "data" / "multi_asset" / "8q_instance" / "benchmark" / "three_asset_instance.npz"
...
np.savez_compressed(
    ...
    M=np.array(M),
    n_bits_small=np.array([2, 2, 2], dtype=int),
