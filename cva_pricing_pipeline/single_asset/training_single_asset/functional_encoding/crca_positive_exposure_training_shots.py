...
v_s_t: np.ndarray = benchmark["v_joint_t"]   
C_v: float = float(benchmark["C_v"])

f_target: np.ndarray = v_s_t.flatten(order="C") / C_v   # shape (16,)
...
ctrl_labels = [
    f"t={format(idx % M, f'0{m_time}b')},s={format(idx // M, f'0{n_price}b')}"
...
fig_dist, fig_cost = plot_training_diagnostics(
    ...
    best_idx=best_idx,
    labels=ctrl_labels,
    xlabel="Control register |t,s⟩",
    ylabel="f(t,s)",
    cost_ylabel=(
        f"L2 loss  (SPSA, LR={learning_rate:.4g},"
        f" PERT={perturbation:.4g}, shots={shots})"
    ),
    title_before="Before training  (CRCA positive exposure, SPSA shots)",
    title_after="After training  (best-iter, CRCA positive exposure, SPSA shots)",
    cost_log_x=False,
    cost_log_y=True,
)
plt.show()
...
metadata = {
    ...
    "note": (
        "CRCA positive-exposure training via shots-only SPSA. "
        "Control index convention: i = time + 4*price (time in low bits). "
