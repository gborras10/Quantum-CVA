...
class SPSAHyperparamCV:
    ...
    def __init__(
        ...
        *,
        ...
        self.tail_eval_shots = (
            int(tail_eval_shots) if tail_eval_shots is not None else None
        )

    def _refine(self, val: float) -> list[float]:
        return sorted(
            {float(val * f) for f in self.refine_factors if (val * f) > 0.0}
        )
    ...
    def fit(
        self,
        *,
        make_cost_train: Callable[
            [int, float | None], Callable[[np.ndarray], float]
        ],
        make_cost_eval: Callable[
            [int, float | None], Callable[[np.ndarray], float]
        ],
    ) -> SPSACVResult:
        ...
        if use_eps:
            for lr, pert, eps in product(self.lr_grid, self.pert_grid, self.eps_grid):  # type: ignore[arg-type]
                ...
                if self.verbose:
                    print(
                        f"[coarse] lr={lr:.4f}, pert={pert:.4f}, eps={epsf:.1e} -> eval={val:.3e}"
                    )
        else:
            for lr, pert in product(self.lr_grid, self.pert_grid):
                ...
                if self.verbose:
                    print(
                        f"[coarse] lr={lr:.4f}, pert={pert:.4f} -> eval={val:.3e}"
                    )
        ...
        if use_eps:
            for _, lr0, pert0, eps0 in coarse_topk:
                ...
                for lr, pert, eps in product(lr_f, pe_f, ep_f):
                    ...
                    if self.tail_eval_shots is not None:
                        cost_eval_tail = make_cost_eval(
                            self.tail_eval_shots, epsf
                        )
                    else:
                        cost_eval_tail = cost_eval_main
                    ...
                    if self.verbose:
                        print(
                            f"[fine]   lr={lr:.6f}, pert={pert:.6f}, eps={epsf:.1e} -> eval={val:.3e}"
                        )
        else:
            for _, lr0, pert0 in coarse_topk:
                ...
                for lr, pert in product(lr_f, pe_f):
                    ...
                    if self.verbose:
                        print(
                            f"[fine]   lr={lr:.6f}, pert={pert:.6f} -> eval={val:.3e}"
                        )
        ...
        if self.verbose:
            ...
            print(
                f"  tail_mode={self.tail_mode}, tail_window={self.tail_window}, tail_eval_shots={self.tail_eval_shots}"
            )
