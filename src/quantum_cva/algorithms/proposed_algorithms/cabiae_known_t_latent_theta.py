...
Key changes relative to the original CABIQAE implementation:
1. The stage-to-stage prior is prepared by inferring a latent density on theta
   over the current identifiable interval and propagating it forward through the
   noisy observation model, instead of inverting observed probabilities
   sample-by-sample.
2. Beta credible intervals in each stage are converted to theta credible
   intervals through the same latent-theta representation.
3. The next Grover depth is selected with a noise-aware score based on expected
   Fisher information per unit oracle cost, while retaining the IQAE
   identifiability constraint and the hard depth cap.
"""
...
class CABIQAELatentTheta(AmplitudeEstimator):
    ...
    This variant keeps the observable-vs-ideal probability split in the physics
    model, but moves the Bayesian transport between stages to the latent angle
    ``theta``. This avoids the unstable stage-to-stage map based on direct
    inversion of the observed noisy probability.
    """
    ...
    def __init__(
        ...
        valid_scheduler_modes = {"expected_fisher", "contrast_linear"}
    ...
    def _expected_fisher_score(self, k: int, theta_interval: tuple[float, float]) -> float:
        ...
        if self._scheduler_mode == "contrast_linear":
            score = (self._contrast(k) ** 2) * (2 * k + 1)
            return float(score)
        return float(np.mean(fisher_theta) / max(2 * k + 1, 1))
    ...
    def _get_prior(
        ...
        A density on theta is reconstructed over the current identifiable interval
        from the current stage posterior on the observed probability. Samples from
        this theta density are pushed forward through the next-stage noisy model,
        and a Beta prior is fitted to the resulting observed probabilities.
        """
