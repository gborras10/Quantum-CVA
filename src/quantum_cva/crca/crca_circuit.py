# src/quantum_cva/crca/crca_circuit.py
...
class CrcaCircuit:
    ...
    def bind(self, x: np.ndarray, *, measured: bool = False) -> QuantumCircuit:
        ...
        template = self._qc_eval_meas if measured else self.qc_eval
    ...
    def function_values(
        ...
        *,
        ...
        if shots is None:
            ...
            probs = np.asarray(sv.probabilities(), dtype=float)  # length 2**(n_controls+1)
        ...
        bind_map = {self.theta[i]: float(x[i]) for i in range(self.n_params)}
        tqc_bound = self._tqc_eval_meas.assign_parameters(bind_map, inplace=False)
    ...
    def _build_eval_circuits(self) -> tuple[QuantumCircuit, QuantumCircuit]:
        ...
        for q in controls_q:
            qc_meas.h(q)
        qc_meas.compose(self.qc, qubits=list(qc_meas.qubits)[: len(qc_eval.qubits)], inplace=True)
    ...
    def _extract_clbit_indices(self, qc_meas: QuantumCircuit) -> tuple[list[int], int]:
        ...
        if "c" not in cregs or "ca" not in cregs:
            raise RuntimeError("Expected classical registers 'c' and 'ca' in eval circuit.")
